...
namespace kcp2k
{
    public abstract class KcpPeer
    {
        ...
        protected KcpPeer(KcpConfig config, uint cookie)
            : this(config, cookie, 0)
        {
        }

        // Socketless users already have a monotonic clock (and often need one clock
        // shared by many peers), so the session supplies the initial value and each
        // Tick call supplies the current value.
        protected KcpPeer(KcpConfig config, uint cookie, uint initialTime)
        {
            // initialize variable state in extra function so we can reuse it
            // when reconnecting to reset state
            Reset(config, initialTime);
        ...
        protected void Reset(KcpConfig config, uint initialTime)
        {
            // reset state
            cookie = 0;
            state = KcpState.Connected;
            lastReceiveTime = initialTime;
            lastPingTime = initialTime;
        ...
        /// <summary>Sets the caller-owned clock origin before the first tick.</summary>
        protected void SetInitialTime(uint time)
        {
        ...
        // Gives socketless wrappers a chance to commit handshake state only after the
        // reliable Hello has reached the application receive path.
        protected virtual bool OnReliableHelloReceived() => true;
        ...
        void HandleTimeout(uint time)
        {
            ...
            if (Utils.TimeDiff(time, lastReceiveTime) >= timeout)
            {
        ...
        void HandlePing(uint time)
        {
            ...
            if (Utils.TimeDiff(time, lastPingTime) >= PING_INTERVAL)
            {
        ...
        bool ReceiveNextReliable(uint time, out KcpHeaderReliable header, out ArraySegment<byte> message)
        {
            message = default;
        ...
        void TickIncoming_Connected(uint time)
        {
            ...
            if (remoteDisconnectRequested)
            {
                remoteDisconnectRequested = false;
                Disconnect();
            }
        }
        ...
        void TickIncoming_Authenticated(uint time)
        {
            ...
            // The peer's terminal reason, if any, was delivered by the reliable receive loop
            // above; now honor the disconnect control that arrived with it.
            if (remoteDisconnectRequested)
            {
                remoteDisconnectRequested = false;
                Disconnect();
        ...
        public virtual void TickIncoming(uint time)
        {
            currentTime = time;
            try
            {
        ...
        public virtual void TickOutgoing(uint time)
        {
            currentTime = time;
            try
            {
        ...
        // Process both directions using one caller-owned monotonic timestamp. Keeping the
        // incoming pass first matches the original client/server tick order.
                ...
                case KcpHeaderUnreliable.Disconnect:
                    {
                        ...
                        // Defer to the end of this incoming tick: TickIncoming receives the
                        // reliable messages that were already buffered before honoring this.
                        remoteDisconnectRequested = true;
                        break;
                    }
            }
        }
        ...
        bool SendReliable(KcpHeaderReliable header, ArraySegment<byte> content)
        {
        ...
        bool SendUnreliable(KcpHeaderUnreliable header, ArraySegment<byte> content)
        {
        ...
        public bool SendData(ArraySegment<byte> data, KcpChannel channel)
        {
            ...
            switch (channel)
            {
                case KcpChannel.Reliable:
                    return SendReliable(KcpHeaderReliable.Data, data);
                case KcpChannel.Unreliable:
                    return SendUnreliable(KcpHeaderUnreliable.Data, data);
                default:
                    OnError(ErrorCode.InvalidSend, $"{GetType()}: unknown channel {(byte)channel}.");
                    return false;
            }
        }
        ...
        // A peer's disconnect control is honored only after this tick delivered every reliable
        // message KCP already buffered. One input batch can carry both the peer's terminal data
        // and its disconnect header; acting on the header first would discard the reason it sent
        // before leaving.
        bool remoteDisconnectRequested;

        // disconnect this connection
        public virtual void Disconnect()
        {
            // only if not disconnected yet
            if (state == KcpState.Disconnected || disconnecting)
                return;

            // RawSend is synchronous. Claim ownership before invoking it so a transport
            // callback that closes the session cannot recursively send another goodbye.
            disconnecting = true;
