...
namespace CardShopCoop.Net
{
    ...
    internal sealed class EnumWireConverter<T> : JsonConverter where T : struct
    {
        ...
        public EnumWireConverter(Util.EnumKind kind) { _kind = kind; }
        ...
        {
            if (reader.TokenType == JsonToken.Null) throw new JsonSerializationException("Null enum " + objectType);
    ...
    {
        public override bool CanConvert(Type t) { return t == typeof(Vector3); }
        public override void WriteJson(JsonWriter w, object value, JsonSerializer s)
        { var v = (Vector3)value; w.WriteStartArray(); w.WriteValue(v.x); w.WriteValue(v.y); w.WriteValue(v.z); w.WriteEndArray(); }
        public override object ReadJson(JsonReader r, Type t, object existing, JsonSerializer s)
        { var a = ReadArray(r, 3); return new Vector3(Convert.ToSingle(a[0], CultureInfo.InvariantCulture), Convert.ToSingle(a[1], CultureInfo.InvariantCulture), Convert.ToSingle(a[2], CultureInfo.InvariantCulture)); }
        private static List<object> ReadArray(JsonReader r, int count)
        { var a = new List<object>(); if (r.TokenType != JsonToken.StartArray) throw new JsonSerializationException("Expected vector array"); while (r.Read() && r.TokenType != JsonToken.EndArray) a.Add(r.Value); if (a.Count != count) throw new JsonSerializationException("Wrong vector component count"); return a; }
    }

    internal sealed class QuaternionConverter : JsonConverter
    {
        public override bool CanConvert(Type t) { return t == typeof(Quaternion); }
        public override void WriteJson(JsonWriter w, object value, JsonSerializer s)
        { var q = (Quaternion)value; w.WriteStartArray(); w.WriteValue(q.x); w.WriteValue(q.y); w.WriteValue(q.z); w.WriteValue(q.w); w.WriteEndArray(); }
        public override object ReadJson(JsonReader r, Type t, object existing, JsonSerializer s)
        { var a = new List<object>(); if (r.TokenType != JsonToken.StartArray) throw new JsonSerializationException("Expected quaternion array"); while (r.Read() && r.TokenType != JsonToken.EndArray) a.Add(r.Value); if (a.Count != 4) throw new JsonSerializationException("Wrong quaternion component count"); return new Quaternion(Convert.ToSingle(a[0], CultureInfo.InvariantCulture), Convert.ToSingle(a[1], CultureInfo.InvariantCulture), Convert.ToSingle(a[2], CultureInfo.InvariantCulture), Convert.ToSingle(a[3], CultureInfo.InvariantCulture)); }
    }
}
